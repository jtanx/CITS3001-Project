...
As there is no feedback, the run-time of this method will vary from board to board, and can vary significantly from computer to computer. It is typical for it to run in excess of 10 moves per second when run on a fairly modern computer. The worst-case running time for this algorithm is proportional to $O(k(b^n))$ where $b$ is the branching factor (4), $n$ is the depth limit (8) and k is the number of tiles in the sequence. This corresponds to where a complete depth-first search to a depth 8 is required for each move of 8 steps.

Multithreading was introduced fairly late, but it did help to significantly increase the solving speed --- often by a factor of two, where the resources are available. This was achieved by splitting the game tree into 4 subtrees (by moving in the 4 directions) and searching each of the 4 subtrees in parallel. However, this design means that it will use at most 4 (logical) cores, and will often slightly under-utilise them, given that the game tree is not balanced.
...
Next to figuring out which heuristics to use was determining how they should be weighted for optimal performance. This was perhaps the most time consuming step, simply because of the sheer number of possible weight combinations. 

While not the most efficient method, such heuristic weights were found by simply enumerating all possible combinations and observing which ones produced the best results. 
